...
module GestPay
  class TransactionData
    ...
    def ready_to_encrypt?
      !(
      (@attributes[:currency].nil? || @attributes[:currency].to_s.size == 0) ||
      (@attributes[:amount].nil? || @attributes[:amount].to_s.size == 0) ||
      (@attributes[:shop_transaction_id].nil? || @attributes[:shop_transaction_id].to_s.size == 0)
      )
    end

    def encrypt_error
      unless ready_to_encrypt?
        if (@attributes[:currency].nil? || @attributes[:currency].to_s.size == 0)
          {:error_code => 552, :error_description => "currency not valid"}
        elsif (@attributes[:amount].nil? || @attributes[:amount].to_s.size == 0)
          {:error_code => 553, :error_description => "amount not valid"}
        elsif (@attributes[:shop_transaction_id].nil? || @attributes[:shop_transaction_id].to_s.size == 0)
    ...
    def create_from_string attributes
      ...
      @attributes[:custom_info] = nil if @attributes[:custom_info].to_s.size == 0
